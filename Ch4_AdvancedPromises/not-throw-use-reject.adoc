...
そもそも、promiseオブジェクトをRejectedにしたい場合に、
何故`throw`ではなく`reject`した方がいいのでしょうか?

ひとつは`throw`が意図したものか、それとも本当に**例外**なのか区別が難しくなってしまうことにあります。

例えば、Chrome等の開発者ツールには例外が発生した時に自動でbreakし、
デバッガーが起動する機能が用意されています。
...
Promiseの中で`throw`してしまうとこの機能が殆ど使い物にならなくなってしまうでしょう。
...
つまり、この`retPromise`がRejectedになった場合はonRejectedが呼び出されるので、
