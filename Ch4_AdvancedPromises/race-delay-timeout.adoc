...
`abort()` メソッドを外から呼べるようにするために、今までのセクションででてきた<<xhr-promise.js,`getURL`>>を少し拡張して、
XHRを包んだpromiseオブジェクトと一緒にそのXHRを中止するメソッドを持つオブジェクトを返すようにしています。
...
[NOTE]
通常の開発の場合は繰り返し使えるようにそれぞれファイルに分割して定義しておくといいですね。
