...
以下のようなNode.jsの http://nodejs.org/api/fs.html[fs]モジュールを例にしてみたいと思います。
...
`then`と`catch`を持たせていますが、それ以外のインターフェースは全く同じで使い方となっています。
...
`this.lastValue`のような値を保持する必要がなくなる事や大きなファイルの扱いが改善したり、
...
`ArrayAsPromise` の実装見て気づくと思いますが、`Array.prototype`のメソッドを全て実装しています。
...
その機能にPromiseを使うことは最適なのかを考える事はこの書籍の目的でもあるため、
何でもPromiseにするというわけではなく、その目的にPromiseが適当なのかを考えてみるのもいいと思います。
