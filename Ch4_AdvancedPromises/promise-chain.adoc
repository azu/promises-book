...
そのような仕組みは<<ES6Promises>>にはありませんが、
...
次のような`Array`のPromise版を使えるメソッドを動的に定義する例を考えてみましょう。
...
しかし、`array.indexOf`など`Array.prototype`には配列を返さないものもあるため、全てをメソッドチェーンにするのは少し不自然なケースがあると思います。

ここで大事なのが、同じ値を受けるインターフェースを持っているAPIはこのような手段で
Promise版のAPIを自動的に作成できるなどの汎用性が高まるので、そのような点を意識してみるとまた違った使い方が見つかるかもしれません。
