...
ここでは、 http://mochajs.org/[Mocha]自体については詳しく解説しませんが、
...
それでは最初に、コールバック関数のテストと同じような形でテストを書いてみましょう。
...
このテストを `basic-test.js` というファイル名で作成した場合は、
先ほどインストールしたMochaをコマンドラインで実行することでテストができます。
...
`done()` が呼ばれるまでテストケースの終了を待つことで非同期のテストをサポートしています。
