...
そのため、Web Notifications APIではNoticationのモックオブジェクトを作成し、
テストをおこなえるようにした。
...
これにより、JavaScriptの文法として間違っているもはパースエラーとなるため、
インラインに書いたコードのミスを検出するのに役だった。
...
リソースが欠損しててもエラーではなくWARNINGとなるため、
ビルドするときにWARNINGが発生したらCIが落ちるようにした。
