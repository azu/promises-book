...
サンプルコードはNode.js上で動くものとして作り、一般的な手法でテストを書いた。
サンプルコードから表示用コードを自動的に生成するが、表示用コードはブラウザで実行を前提としている。
そのためNode.jsに限定された内容以外は、どちらの環境でも動く書き方が必要となった。

多くはDOM APIがその対象だが、具体的には`XMLHttpRequest`や`Notification`等が該当する。
