...
resolveされたことにより _promiseオブジェクトがFulFilledの状態になった_ 時という事ですね。
...
<<xhr-promise.js,getURL関数>> 内で `resolve(req.responseText);` によってpromiseオブジェクトが解決されると、
...
rejectされたことより _promiseオブジェクトがRejectedの状態になった_ 時という事ですね。

**reject**された時の処理は、<<promise.then,`.then` の第二引数>> または <<promise.catch,`.catch`>> メソッドに呼びたい関数を渡す事で行えます。
